...
import Products1 from '../model/Course.js'

export const list = (req, res) => {
  Products1.find().then(result =>  res.json(result));
};

// add new product 
export const create = (req, res) => {
  Products1.insert(req.body).then(()=> {return res.send({success: "Create Successfully"})})
}

// Find a single product with an id
export const get = (req, res) => {
    const id = req.params.id;
    Products1.findOne(id)
   .then(product1 => {
      if(!product1) {
      ...
      res.json(product1); // default status = 200
    }).catch(err => {
...
export const put = (req, res) => {
    ...
    Products1.findAndUpdate(req.params.id,req.body,true)
    .then(product1 => {
      if(!product1) {
        return res.status(404).send({
